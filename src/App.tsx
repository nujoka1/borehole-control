...
function SettingsPanel({ settings, onSave }: { settings: DeviceSettings | null; onSave: (payload: Record<string, unknown>) => Promise<void> }) {
  ...
  return <section className="card settings-card"><div className="settings-title"><span><Radio/></span><div><h2>Pump thresholds</h2><p>The ESP32 starts the pump at the lower level and stops it at the upper level.</p></div></div><div className="settings-fields"><label>Start filling<div><input type="number" min="0" max="95" value={lower} onChange={event => setLower(Number(event.target.value))}/><span>%</span></div></label><label>Stop filling<div><input type="number" min="5" max="100" value={upper} onChange={event => setUpper(Number(event.target.value))}/><span>%</span></div></label><label>Usable tank depth<select value={depth} onChange={event => setDepth(Number(event.target.value))}>{depths.map(value => <option key={value}>{value} cm</option>)}</select></label></div>{!valid && <p className="validation" role="alert">Use 0–100%, at least a 5% gap, and a supported depth.</p>}<button className="primary" disabled={!valid || saving} onClick={() => void save()}>{saving ? 'Saving…' : 'Save pump thresholds'}</button></section>
}
